...
getSinglePortfolio = () => {
  ...
  fetch(`https://beta.stockzoom.com/api/v1/me/portfolios/${this.props.match.params.id}/`, {
    ...
    .then(userData => {
      ...
      }, () => console.log("Single Portfolio Details: ", userData))
    })
    .catch(err => {
  ...
  return (
    <div className="myPageView">
      ...
      <div className="portfolioList-container--single">
        <div className="singlePortfolioOverview">
          ...
          <p>
            ...
            {availableCash && `${parseInt(availableCash, 10)} ${currency}`}
          </p>
        </div>
      </div>
          ...
          <div className="portfolioList-container--single">
            {portfolioPositions.map((position, index) => (
              <div key={index} className="position-container">
                  ...
                  <table>
                    ...
                    <tbody>
                      <tr>
                        <td>Today:</td>
                        <td>
                          {position.instrument.price_today}
                          {" "}
                      ...
                      <tr>
                        <td>Open:</td>
                        <td>
                          {position.instrument.price_open}
                          {" "}
                          {position.instrument.currency}
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td>Close:</td>
                        <td>
                          {position.instrument.price_close}
                          {" "}
                          {position.instrument.currency}
                        </td>
                      </tr>
                ...
                <p>
                  {position.instrument.company && position.instrument.company.description}
                </p>
              </div>
            ))}
          </div>)
